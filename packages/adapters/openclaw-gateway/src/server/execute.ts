...
function buildWakeText(
  ...
  const lines = [
    ...
    "   - If no assigned issue is actionable and your role from /api/agents/me is ceo, GET /api/agents/me/goals-review.",
    "   - Review only goals from /api/agents/me/goals-review; do not browse all unassigned work.",
    "   - For each active/planned goal with recommendedAction needs_planning_issue, POST /api/companies/{companyId}/issues with goalId, a linked projectId when present, status todo, title \"Plan execution path for: {goal.title}\", and a description containing the goal description and ancestry.",
    "   - For needs_unblock or needs_delegation, update/comment/delegate on the existing execution issue instead of creating duplicate planning work.",
    "   - Exit only after no assigned issues, no valid handoff, and no goal-review actions remain.",
    "",
